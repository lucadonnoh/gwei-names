import {
  ...
  verifyHash,
  yParityToV
} from "./chunk-OH4ES7LS.js";
import "./chunk-4I67GBXX.js";
import {
  BaseError,
  BaseError2,
  FeeCapTooHighError,
...
} from "./chunk-3XVEUS3D.js";
import "./chunk-TD4VXY2C.js";
import {
  __export
} from "./chunk-7DQDWJI5.js";
...
function verify2(options) {
  ...
  return secp256r1.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex2(publicKey).substring(2), { lowS: true, ...hash2 ? { prehash: true } : {} });
}
...
function assert4(envelope) {
  ...
  if (type2 === "p256") {
    ...
    const missing = [];
    if (typeof p2562.signature?.r !== "bigint")
      missing.push("signature.r");
    if (typeof p2562.signature?.s !== "bigint")
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "p256") {
    const p2562 = envelope;
    return concat2(serializedP256Type, fromNumber(p2562.signature.r, { size: 32 }), fromNumber(p2562.signature.s, { size: 32 }), fromNumber(p2562.publicKey.x, { size: 32 }), fromNumber(p2562.publicKey.y, { size: 32 }), fromNumber(p2562.prehash ? 1 : 0, { size: 1 }), options.magic ? magicBytes : "0x");
  }
  if (type2 === "webAuthn") {
    const webauthn = envelope;
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload2(from7(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload2(from7(envelope)),
...
function getSignPayload2(envelope, options = {}) {
  ...
  if (options.from)
    return keccak256(concat2("0x04", sigHash, resolve(options.from)));
  return sigHash;
}
