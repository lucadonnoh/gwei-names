...
/// @notice Tests SubdomainRegistrar against the Gwei Name Service (NameNFT) on a mainnet fork.
///         gwei isn't deployed on mainnet yet, so the parent NameNFT is deployed fresh and
///         injected into the registrar via constructor; everything else — including real
///         mainnet USDC for the ERC20 fee / gating tests — is unchanged from upstream.
///         Covers escrow mode, flash mode, ETH/ERC20 fees, gating, withdrawals, and all
///         revert paths.
contract SubdomainRegistrarTest is Test {
    ...
    function setUp() public {
        ...
        nft = new NameNFT();
        nameNFT = INameNFTFull(address(nft));
    ...
    function _registerName(string memory label, address to) internal returns (uint256 tokenId) {
        ...
        tokenId = nameNFT.computeId(string.concat(label, ".gwei"));
    ...
    function testRenewEscrowedParentThenWithdraw() public {
        ...
        vm.prank(controller);
        nameNFT.renew{value: fee}(parentId);
