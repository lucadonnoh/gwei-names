...
export default {
  async fetch(request) {
    ...
    // 1. Resolve the name's contenthash on-chain.
    const idRes = await ethCall(encodeString(SEL_COMPUTEID, name));
    if (!idRes) return page('gwei gateway', '<p>Resolution failed (RPC).</p>', 502);
    const chRes = await ethCall('0x' + SEL_CONTENTHASH + idRes.slice(2));
    if (!chRes) return page('gwei gateway', '<p>Resolution failed (RPC).</p>', 502);
    ...
    return page(name, '<p>Content is set but couldn’t be fetched from IPFS right now.</p>', 504);
  },
};
